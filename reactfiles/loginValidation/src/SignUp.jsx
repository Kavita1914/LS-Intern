...
const handleSubmit = async () => {
  try {
    ...
    if (isValid) {
      // Extract only the desired fields from formFields
      const userToSave = {
      ...
      // Retrieve the current users from local storage
  ...
  return (
    <div className="mainDiv">
     ...
     <div>
        <Link to="/login">
        <button className="btn2">Already have an account ? <span>  Log In</span></button>
        </Link>      
        </div>
    </div>
  );
